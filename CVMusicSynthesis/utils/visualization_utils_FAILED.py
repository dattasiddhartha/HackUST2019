...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    ...
    #   image = np.array(Image.fromarray(np.uint8(image)).convert("LA")) 
#     image_copy = image.copy()
    
    # Convert to greenscale
#     r, g, b = cv2.split(image)
#     b = b.astype("float32") * 0.114
#     g = g.astype("float32") * 0.587
#     r = r.astype("float32") * 0.299
...
#   for box_color, this_class in zip(box_to_color_map.items(), classes):
#     box = box_color[0]
#     color = box_color[1]
    for box, color in box_to_color_map.items():
      ymin, xmin, ymax, xmax = box
    
#     if desired_class == this_class:
